...
const increment = () => {
  emit('update:count', props.thread.thread_count + 1);
};

const decrement = () => {
  if (props.thread.thread_count > 0) {
    emit('update:count', props.thread.thread_count - 1);
  }
};
</script>
...
<template>
  <div class="relative flex items-center group">
    ...
    <!-- Main Card -->
    <div class="w-40 bg-gray-800/40 backdrop-blur-md border border-white/10 rounded-4xl overflow-hidden flex flex-col text-white transition-all duration-500 hover:scale-[1.02] hover:bg-gray-800/60 hover:border-indigo-500/50 hover:shadow-[0_20px_50px_rgba(79,70,229,0.15)]">
      <!-- Header: ID -->
      ...
      <!-- Body: Count -->
      <div class="grow flex items-center justify-center py-3 relative">
      ...
      <div class="pt-1 pb-5 text-center">
        <span class="text-[10px] font-black uppercase tracking-[0.5em] text-gray-500/80">
          {{ thread.brand }}
        </span>
      </div>
    </div>
    ...
    <div class="absolute left-full top-8 flex flex-col gap-2 ml-1 z-10">
      ...
      <div 
        ...
        @click="emit('edit')"
      >
        <div class="min-w-7 flex items-center justify-center opacity-0 group-hover/btn:opacity-100 transition-opacity duration-300">
